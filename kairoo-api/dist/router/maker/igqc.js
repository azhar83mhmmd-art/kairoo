...
async function igqcHandler(req, res) {
    ...
    try {
        await (0, _canvas_1.ensureRemoteFont)(FONT_URL, FONT_FAMILY, 'igqc-inter-regular.woff2');
        const bg = await (0, _canvas_1.remoteImage)(BG_URL);
        ...
        if (hasImg) {
            try {
                imgObj = await (0, _canvas_1.remoteImage)(fotoUrl);
            }
            catch {
        ...
        while (chatFontSize >= minFontSize) {
            if (hasImg && hasTxt && imgObj) {
                ...
                chatLines = await (0, _emoji_1.wrapTextWithEmojis)(ctx, teks, maxWidthLimit, chatFontSize);
        ...
        if (hasTxt) {
            ...
            for (const l of chatLines) {
                const w = await (0, _emoji_1.measureTextWithEmojis)(ctx, l.trim(), chatFontSize);
                if (w > longestW)
                    longestW = w;
            }
            ...
            for (let i = 0; i < chatLines.length; i++) {
                ...
                await (0, _emoji_1.drawTextWithEmojis)(ctx, chatLines[i].trim(), fixedX + paddingX, lineY, chatFontSize);
            }
            ctx.restore();
        }
        const emojis = ['❤️', '😂', '😮', '😢', '😡', '👍'];
        ...
        for (let i = 0; i < Math.min(emojis.length, 6); i++) {
            await (0, _emoji_1.drawTextWithEmojis)(ctx, emojis[i], startX + i * spacingX - emojiSize / 2, emojiCY, emojiSize);
        }
