...
exports.default = kalenderHandler;
...
async function ensureFonts() {
    ...
    for (const f of FONTS) {
        try {
            await (0, _canvas_1.ensureRemoteFont)(f.url, f.family, f.file);
...
const F = (family) => FALLBACK[family] || 'sans-serif';
...
function renderTahun() {
    ...
    const canvas = (0, canvas_1.createCanvas)(lebarKanvas, tinggiKanvas);
...
async function kalenderHandler(req, res) {
    ...
    if (mode === 'bulan') {
        ...
        if (!nomorBulan) {
            return res.status(400).json({
                status: false,
                message: "Parameter 'bulan' wajib diisi salah satu dari: januari, februari, maret, april, mei, juni, juli, agustus, september, oktober, november, desember."
            });
        }
    }
    try {
        await ensureFonts();
        const canvas = mode === 'tahun' ? renderTahun() : renderBulan(nomorBulan);
