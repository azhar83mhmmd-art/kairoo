...
const detectPlatform = (url) => {
    ...
    if (/(^|\.)(twitter\.com|x\.com)$/.test(host))
        return 'twitter';
    if (/(^|\.)pinterest\.(com|[a-z]{2,3})$|(^|\.)pin\.it$/.test(host))
        return 'pinterest';
...
const downloadTiktok = async (url) => {
    const { data } = await axios_1.default.get('https://www.tikwm.com/api/', {
        params: { url, hd: 1 },
        timeout: 15000,
        headers: {
    ...
    if (d.hdplay || d.play)
        result.video = d.hdplay || d.play;
    if (d.play)
        result.video_watermark = d.wmplay;
    if (d.music)
        result.audio = d.music;
    return result;
};
async function aioHandler(req, res) {
    ...
    const isPrivateHost = blockedHosts.includes(parsed.hostname) ||
