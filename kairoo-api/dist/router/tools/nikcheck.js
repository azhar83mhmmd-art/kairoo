...
exports.default = nikCheckHandler;
...
const findByCode = (list, code) => list.find((item) => item.code === code)?.name;
async function getRegionNames(idProv, idKabKota, idKec) {
    ...
    try {
        const { data } = await axios_1.default.get(`${WILAYAH_BASE}/provinces.json`, { timeout: 8000 });
    ...
    try {
        const { data } = await axios_1.default.get(`${WILAYAH_BASE}/regencies/${provCode}.json`, { timeout: 8000 });
        result.kabupatenKota = findByCode(extractList(data), kabCode) || result.kabupatenKota;
    }
    catch { }
    try {
        const { data } = await axios_1.default.get(`${WILAYAH_BASE}/districts/${kabCode}.json`, { timeout: 8000 });
        result.kecamatan = findByCode(extractList(data), kecCode) || result.kecamatan;
    }
    catch { }
    return result;
}
async function nikCheckHandler(req, res) {
